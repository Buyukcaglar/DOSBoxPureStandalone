...
The reveal logic must also support any package whose user must see an intentional DOS text display. This includes games that remain in text mode and graphical games with interactive text-mode questions before graphics begin. Such a package declares this behavior with package metadata `text_mode: true`; a legacy empty root-level `TEXTMODE.DBP` marker must remain compatible. The display must become visible after a short text-mode dwell and replacement of the shell display, or after a bounded fallback. Without either declaration, noninteractive text-page or text-resolution changes made by a graphics game during initialization must never expose a transitional DOS frame, regardless of their duration.
...
The builder must validate the exact final size before appending and reject any
output of 4 GiB or larger because Windows will not launch such an executable.
The error must state the maximum archive size available with the selected
runtime resources and suggest size reduction or a multi-file distribution.
...
boolean manifest field named `text_mode`. Enabling either must emit
`"text_mode": true` in package metadata so the runtime text-presentation path
is used.
...
as Sid Meier's Civilization. It must not be required for noninteractive
transitional text, such as Beneath a Steel Sky startup behavior.

The builder must store the source ZIP/DOSZ byte-for-byte unchanged. It must not
modify the archive, extract archive members, or write a reconstructed archive
to disk. The runtime must continue accepting a legacy root-level `TEXTMODE.DBP`
marker. Packages without a metadata field or legacy marker must retain their

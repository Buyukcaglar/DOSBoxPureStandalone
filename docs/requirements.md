...
The reveal logic must also support any package whose user must see an intentional DOS text display. This includes games that remain in text mode and graphical games with interactive text-mode questions before graphics begin. Such a package declares this behavior with an empty root-level `TEXTMODE.DBP` marker, whether supplied by the source archive or generated by the package builder, and must become visible after a short text-mode dwell and replacement of the shell display, or after a bounded fallback. Without the marker, noninteractive text-page or text-resolution changes made by a graphics game during initialization must never expose a transitional DOS frame, regardless of their duration.

...
- a package that requires an intentional DOS text display opts in with package metadata `"text_mode": true`, whether it remains in text mode or later enters graphics
- `makegame --text-mode` and manifest `"text_mode": true` emit that metadata while leaving the source and stored ZIP/DOSZ bytes identical; legacy archives with an empty root-level `TEXTMODE.DBP` marker remain compatible
...
Windows accepts a PE file with appended data only while the total executable
size remains below 4 GiB. A file of 4,294,967,295 bytes launches, while a file
of 4,294,967,296 bytes is rejected by `CreateProcess` before runtime code can
execute. The builder therefore calculates the exact post-resource-update
capacity before appending and rejects any package that would reach that hard
operating-system ceiling.
...
2. validate every ZIP/DOSZ member and calculate archive identity as streams
3. embed package metadata, including optional `text_mode`
4. store the DOSZ/ZIP archive as resource 101 or stream it to an appended mapped payload
...
ZIP64 archives are appended after resource updates and followed by a fixed
trailer; the PE image remains small enough for the Windows loader. Validation,
identity calculation, appended writing, and final byte comparison use bounded
buffers rather than a whole-archive byte array. With `--text-mode` or manifest
`text_mode`, the builder emits `"text_mode": true` in resource 102. It does not
alter the source archive, extract its members, or reconstruct an archive on
disk. The legacy root-level `TEXTMODE.DBP` declaration remains supported.
The resource-updated runtime, archive, and 32-byte trailer must together remain
smaller than 4 GiB; the builder reports the exact archive capacity for the
selected template, configuration, metadata, and icon when this is exceeded.
...
```text
--full-screen                      -> screen_fullscreen true
omitted                             -> screen_fullscreen false
--lock-mouse                       -> interface_lockmouse true
omitted                             -> interface_lockmouse false
...
`--full-screen` is a boolean switch: when present it writes
...
higher precedence. `--lock-mouse` applies the same rule to
`interface_lockmouse`: present writes `true` and captures the pointer at startup,
while omitted writes `false`. The runtime's default Ctrl+F11 hotkey can still
toggle that state. The aspect-ratio switch exposes
...
The first boolean fullscreen replacement used `--fullscreen`, rejected the
former `--window-mode`, valued syntax, and repetition, and serialized explicit
`true` or `false` values into resource 103. It was subsequently renamed to the
hyphenated `--full-screen` spelling; current validation is recorded with the
mouse-lock package-default tests below.

Fullscreen and mouse-lock validation accepted `--full-screen` and
`--lock-mouse` independently and together. Built-in help contained those exact
spellings and excluded `--fullscreen`; the old spelling, valued syntax, and
repeated boolean switches were rejected with exit code 2. Four generated
packages covered both switches present and absent, with contradictory config
values in both directions. Direct resource 103 inspection confirmed exact
`screen_fullscreen` and `interface_lockmouse` values, preserved an unrelated
memory setting, and left the source archive hash unchanged. A mouse-locked
runtime package and a package generated by the self-contained builder both
created their memory-backed overlays and exited with code 0. The self-contained
builder also passed with machine-wide .NET discovery disabled.

Current text-mode packaging validation generated a package with `--text-mode`,
confirmed `text_mode: true` in resource 102, and confirmed resource 101 was
byte-for-byte identical to the source archive. The source hash remained
unchanged and the runtime smoke package exited with code 0. The core retains
the earlier root-level `TEXTMODE.DBP` behavior for packages created by older
builders.

Large-package validation used a 2,202,009,833-byte ZIP with a stored entry
larger than 2 GiB. The builder validated, identified, appended, and compared it
using bounded buffers, produced a 2,204,930,313-byte executable in 6.52
seconds, and the generated runtime exited with code 0. Process Monitor showed
the archive being read from the executable itself and no archive reconstruction
or game-content extraction in Temp, AppData, or the execution directory.
Expected application writes were limited to persistence-root writability probes;
graphics-driver cache activity remained outside project ownership.

Windows boundary tests independently launched a 4,294,967,295-byte PE and
rejected a 4,294,967,296-byte PE. The 4,449,418,984-byte Ripper ZIP therefore
failed exact-capacity validation with a projected executable size of
4,452,479,752 bytes and no requested output or temporary archive left behind.
Its six disc images are single-track MODE1/2352 BIN files; converting compatible
raw images to 2048-byte-sector ISO files is a package-authoring option, not an
automatic builder transformation.

The six Ripper images were then converted sector-by-sector after validating
every sync header and MODE1 byte. The rewritten archive contained ISO primary
volume descriptors `RIPPER1` through `RIPPER6`, measured 3,963,765,347 bytes,
and produced a 3,966,826,115-byte package. DOSBox Pure mounted all six internal
